...
if __name__ == '__main__':
    configuration.activate_config_file('collector')

    current_path = os.path.dirname(os.path.realpath(__file__))
    env = GymWrapper(current_path + '/griddly_descriptions/' + configuration.griddly_description,
    ...
    with cProfile.Profile() as pr:
        cem = CEM(env, configuration.agents)
        for _ in range(1):
            action = cem.cem_action(env, 2, configuration.n_step)
